...
class ComponentTwitterQueryTest extends PHPUnit_Framework_TestCase {
	...
	public function testConstruct() {
		...
		$query = new api\ComponentTwitterQuery($this->location_network, 'location');

		$this->assertEquals('api\ComponentTwitterQuery', get_class($query));
	}
	...
	public function testLocationNetworkBuildSearch() {
		...
		$this->assertEquals(urldecode('https://api.twitter.com/1.1/search/tweets.json?q=(#UnitedStates OR "United States") -filter:retweets&result_type=mixed&since=2010-01-01'), urldecode($query->getSearch()));
	}

	public function testLocationBuildAndUntil() {

		$this->location_network->query_location_scope = 3;
		$query = new api\ComponentTwitterQuery($this->location_network, 'location');
		$query->includeUntilDate('2015-07-01');

		$this->assertEquals(urldecode('https://api.twitter.com/1.1/search/tweets.json?q=(#UnitedStates OR "United States") -filter:retweets&result_type=mixed&since=2010-01-01&until=2015-07-01'), urldecode($query->getSearch()));
	}

	public function testLanguageNetworkBuildSearch() {

		$query = new api\ComponentTwitterQuery($this->language_network, 'origin');

		$this->assertEquals(urldecode('https://api.twitter.com/1.1/search/tweets.json?q=(Chinese) -filter:retweets&lang=zh-tw&result_type=mixed&since=2010-01-01'), urldecode($query->getSearch()));
	}
	...
	public function testHashNetworkBuildSearch() {
		...
		$this->assertEquals(urldecode('https://api.twitter.com/1.1/search/tweets.json?q=(%23GrandRapids%20OR%20%22Grand%20Rapids%22) -filter:retweets&result_type=mixed&since=2010-01-01'), urldecode( $query->getSearch() ));
	}
	...
	public function testSlashLanguageNetworkBuildSearch() {
		...
		$this->assertEquals(urldecode('https://api.twitter.com/1.1/search/tweets.json?q=(%23MandarinChinese%20OR%20%22Mandarin%20Chinese%22%20OR%20Putonghua) -filter:retweets&lang=zh-tw&result_type=mixed&since=2010-01-01'), urldecode($query->getSearch()));
	}
	...
	public function testParenthesisLanguageNetworkBuildSearch() {
		...
		$this->assertEquals(urldecode('https://api.twitter.com/1.1/search/tweets.json?q=(%23ASL%20OR%20%22American%20Sign%20Language%22) -filter:retweets&result_type=mixed&since=2010-01-01'), urldecode($query->getSearch()));
	}

	public function testGetLanguageAnyway() {

		$query = new api\ComponentTwitterQuery($this->language_network, 'location');
